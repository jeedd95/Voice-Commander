...
public class JHW_BulletMove : MonoBehaviour
{
    ...
    public Transform Projectile; // 총알
    ...
    bool flag;
    IEnumerator ParabolaBulletMove() //곡사포
    {
        if(flag==false)
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.CompareTag("Enemy") || other.CompareTag("Player")) // 적하고 총알이 부딪혔을때 데미지 계산식
        {
            ...
            if (unitInfo.unitName == "Artillery" || unitInfo.unitName == "Tank" || unitInfo.unitName =="Raptor") //폭발형 유닛들
            {
                GameObject explo= Instantiate(exploArea);
                explo.transform.position = other.transform.position;
                explo.GetComponent<JHW_ExploArea>().SetCreator(this); //폭파 범위에게 총알을 알려주고 싶다
                Destroy(explo, 0.3f);
            }
        ...
        else if(other != null) //뭐에 부딪히든 10초안에는 사라지게
        {
